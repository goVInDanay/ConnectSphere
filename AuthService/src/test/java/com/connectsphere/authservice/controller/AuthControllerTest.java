...
class AuthControllerTest {
	...
	@BeforeEach
	void setUp() {
		sampleUser = User.builder().userId(1).username("johndoe").email("john@example.com").fullName("John Doe")
	...
	void logout_success() throws Exception {
		...
		mockMvc.perform(post("/api/auth/logout").with(csrf()).header("Authorization", "Bearer some-access-token"))
	...
	void getProfileById_found() throws Exception {
		...
		mockMvc.perform(get("/api/auth/profile/1")).andExpect(status().isOk()).andExpect(jsonPath("$.userId").value(1))
				.andExpect(jsonPath("$.username").value("johndoe"));
	}
...
//
//		mockMvc.perform(put("/api/auth/profile").with(csrf()).contentType(MediaType.APPLICATION_JSON)
...
//
//		mockMvc.perform(put("/api/auth/password").with(csrf()).contentType(MediaType.APPLICATION_JSON)
//				.content(objectMapper.writeValueAsString(req))).andExpect(status().isOk())
...
//
//	    mockMvc.perform(delete("/api/auth/deactivate")
//	            .with(csrf())
//	            .principal(() -> "1")) // 👈 FIX
...
//	}
	...
	void searchUsers_success() throws Exception {
		...
		mockMvc.perform(get("/api/auth/search").param("q", "john")).andExpect(status().isOk())
				.andExpect(jsonPath("$.length()").value(1)).andExpect(jsonPath("$[0].username").value("johndoe"));
	}
	...
	void searchUsers_noResults() throws Exception {
		...
		mockMvc.perform(get("/api/auth/search").param("q", "nobody")).andExpect(status().isOk())
	...
	void getEmail_found() throws Exception {
		...
		mockMvc.perform(get("/api/auth/1/email")).andExpect(status().isOk())
				.andExpect(jsonPath("$.email").value("john@example.com"));
	}
	...
	void getUsersByUsernames_success() throws Exception {
		...
		mockMvc.perform(get("/api/auth/username").param("usernames", "johndoe")).andExpect(status().isOk())
